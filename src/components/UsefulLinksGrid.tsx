...
import { Database, Mail, Globe, FileText, Monitor, ExternalLink, LayoutDashboard } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {links.map((link, idx) => {
        ...
        if ('internal' in link && link.internal) {
          return (
            <motion.div key={idx} {...sharedProps}>
              <Link to={link.url} className="w-full flex flex-col items-center">
                {Content}
              </Link>
            </motion.div>
          );
